    ...
    _init(settings) {
      ...
      if (this._settings.get_boolean("symbolic-icon")) {
        ...
        for (let i = 0; i < symbolicChildren; i++) {
          if (i === this._settings.get_int("menu-button-icon-image")) {
            symbolicIconsFlowBox.select_child(
              symbolicIconsFlowBox.get_child_at_index(i)
            );
            break;
          }
        }
      ...
      if (!this._settings.get_boolean("symbolic-icon")) {
        ...
        for (let i = 0; i < children; i++) {
          if (i === this._settings.get_int("menu-button-icon-image")) {
            colouredIconsFlowBox.select_child(
              colouredIconsFlowBox.get_child_at_index(i)
            );
            break;
          }
        }
      ...
      menuButtonIconSizeScale.connect("value-changed", () => {
        this._settings.set_int(
          ...
          menuButtonIconSizeScale.get_value()
        );
      });
      ...
      if (this._settings.get_string("custom-icon-path"))
        customIconPreview.set_from_file(
          this._settings.get_string("custom-icon-path")
        );
  ...
  class LogoMenuOptionsWidget extends Adw.PreferencesPage {
    _init(settings) {
      ...
      menuButtonIconClickTypeCombo.connect("changed", () => {
        this._settings.set_int(
          "menu-button-icon-click-type",
          parseInt(menuButtonIconClickTypeCombo.get_active_id())
        );
      });
      ...
      const extensionApp = this._settings.get_string(
        "menu-button-extensions-app"
      );
      ...
      extensionsAppCombo.connect("changed", () => {
        this._settings.set_string(
          "menu-button-extensions-app",
          extensionsAppCombo.get_active_id()
        );
      });
      ...
      const currentSoftwareCenter = this._settings.get_string(
      ...
      changeSoftwareCenterInput.connect("changed", () => {
        this._settings.set_string(
          "menu-button-software-center",
          changeSoftwareCenterInput.get_text()
        );
      });
      ...
      const currentSystemMonitor = this._settings.get_string(
      ...
      changeSystemMonitorInput.connect("changed", () => {
        this._settings.set_string(
          "menu-button-system-monitor",
          changeSystemMonitorInput.get_text()
        );
      });
      ...
      showPowerOptionsSwitch.set_active(
        this._settings.get_boolean("show-power-options")
      );
      ...
      showGamemodeSwitch.set_active(
        this._settings.get_boolean("show-gamemode")
      );
      ...
      forceQuitOptionsSwitch.set_active(
        this._settings.get_boolean("hide-forcequit")
      );
      ...
      showLockScreenSwitch.set_active(
        this._settings.get_boolean("show-lockscreen")
      );
      ...
      hideSoftwareCentreSwitch.set_active(
        this._settings.get_boolean("hide-softwarecentre")
      );
  ...
  class LogoMenuAboutPage extends Adw.PreferencesPage {
    _init(metadata) {
      ...
      githubLinkRow.add_suffix(
        new Gtk.LinkButton({
          icon_name: EXTERNAL_LINK_ICON,
          uri: "https://github.com/victor7w7r/LogoMenu",
        })
      );
      ...
      contributorRow.add_suffix(
        new Gtk.LinkButton({
          icon_name: EXTERNAL_LINK_ICON,
          uri: "https://github.com/victor7w7r/Logomenu/graphs/contributors",
        })
      );
      ...
      const warrantyLabel = _(
        "This program comes with absolutely no warranty."
      );
      const urlLabel = _(
        "See the %sGNU General Public License, version 2 or later%s for details."
      ).format(
